...
class Evaluator:
    def __init__(self, dataset: Dataset, input_reader: JsonInputReader, text_encoder: BertTokenizer, logger, no_overlapping: bool, no_partial_overlapping: bool,
                 no_duplicate: bool, predictions_path: str, examples_path: str, example_count: int, epoch: int, dataset_label: str,
                 cls_threshold: float, boundary_threshold: float, region_threshold: float, save_prediction = False, path_num = None):
    ...
    def eval_batch(self, entity_logits: torch.tensor, p_left:torch.tensor, p_right:torch.tensor, p_region: torch.tensor, outputs, batch = None):
        batch_size = entity_logits.shape[0] 
        path_num = batch['region_mask'].shape[1]
        ...
        entity_region = p_region.argmax(dim=-1)
        batch_entity_left_scores = p_left.max(dim=-1)[0]
        batch_entity_right_scores = p_right.max(dim=-1)[0]
        batch_entity_region_scores = p_region.max(dim=-1)[0]

        batch_entity_spans = torch.stack([entity_left, entity_right], dim=-1)

        batch_entity_mask = batch_entity_mask * (batch_entity_spans[:,:,0] <= batch_entity_spans[:,:,1]) * (batch_entity_left_scores > self._boundary_threshold) * (batch_entity_right_scores > self._boundary_threshold) * (batch_entity_region_scores > self._region_threshold)
        ...
        for i in range(batch_size):
            ...
            if self._input_reader.entity_type_count < 1000 and self._save_prediction:
                ...
                for entity in doc.entities:
                    # match region
                    if len(entity.match_region)>0:
                        regions = [x['region_index'] for x in entity.match_region if x['region_index'] < path_num]
                    else:
                        regions = [0]
                    ...
                    converted_entity = dict(type=entity_type, start=span_tokens[0].index, end=span_tokens[-1].index, phrase=entity_phrase, region = regions)
                ...
                for j in range(entity_left.size(1)):
                    ...
                    decode_entity["pre_entities"].append(dict(entity_left=entity_left[i][j].item(), entity_right=entity_right[i][j].item(), p_left=roundlist(p_left[i][j].tolist()),
                                                              p_right=roundlist(p_right[i][j].tolist()), phrase=span_tokens,
                                                              ...
                                                              entity_region = entity_region[i][j].item(),
                                                              p_region = roundlist(p_region[i][j].tolist())))
            ...
            entity_region_scores = batch_entity_region_scores[i]
            entity_regions = entity_region[i]
            # #ent
            ...
            valid_region_scores = entity_region_scores[entity_mask]
            valid_entity_regions = entity_regions[entity_mask]

            # valid_entity_scores = valid_entity_scores + valid_left_scores + valid_right_scores

            # [(start, end, entity_type, match_region, cls_score,)]
            sample_pred_entities = self._convert_pred_entities(valid_entity_types, valid_entity_spans, valid_entity_scores, valid_left_scores, valid_right_scores,
                                                               valid_entity_regions, valid_region_scores, doc)
    ...
    def _convert_gt(self, docs: List[Document]):
        for doc in docs:
            ...
            for entity in gt_entities:
                if len(entity.match_region) > 0:
                    regions = [x['region_index'] for x in entity.match_region if x['region_index'] < self._path_num]
                else:
                    regions = [0]
                for region in regions:
                    span_tuple = entity.as_tuple_token()
                    sample_gt_entities.append(span_tuple + (region, ))
            self._gt_entities.append(sample_gt_entities)

    def _convert_pred_entities(self, pred_types: torch.tensor, pred_spans: torch.tensor, pred_scores: torch.tensor,  left_scores, right_scores,
                               pred_region, region_scores,doc):
        converted_preds = []
        ...
        for i in range(pred_types.shape[0]):
            ...
            match_region = pred_region[i].item()
            match_region_score = region_scores[i].item()

            converted_pred = (start, end, entity_type, match_region, cls_score)
            converted_preds.append(converted_pred)
            decode_entity["entities"].append({"start": start, "end": end, "entity_type":entity_type.identifier, "region": match_region,"cls_score": round(cls_score, 2),
                                              "left_score": round(left_score, 2), "right_score": round(right_score, 2), "region_score": round(match_region_score , 2)})
    ...
    def _remove_duplicate(self, entities):
        ...
        for i, can_entity in enumerate(entities):
            ...
            for j, entity in enumerate(non_duplicate_entities):
                #TODO region与span均相同
                if can_entity[0] == entity[0] and can_entity[1] == entity[1] and can_entity[4] == entity[4]:
                    find = True
            if not find:
    ...
    def _convert_by_setting(self, gt: List[List[Tuple]], pred: List[List[Tuple]],
        ...
        def convert(t):
            if only_eeg and len(t)>3:
                c = [t[0], t[1], self._pseudo_entity_type, t[3]]
                return tuple(c)
