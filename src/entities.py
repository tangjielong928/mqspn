...
class Entity:
    def __init__(self, eid: int, entity_type: EntityType, tokens: List[Token], phrase: str, mapping_region: dict):
        ...
        self._tokens = tokens
        self._phrase = phrase
        self.mapping_region = mapping_region
        self.match_region = self.find_region_and_inx()
    ...
    def find_region_and_inx(self):
        regions_list = []
        indx = 0
        for bbox, entity in zip(self.mapping_region['bbox'],self.mapping_region['aspect']):
            if entity == self._phrase:
                regions_list.append({
                    'region_index': indx+1,
                    "bbox": bbox,
                })
...
class Region:
    def __init__(self, regionId: int, image_path, image_id, mapping_region):
        self._regionId = regionId  # ID within the corresponding dataset
    ...
    def region_path(self):
        if self.image_path is not None and self.image_id is not None:
            return  os.path.join(self.image_path, self.image_id)
        return None

    def __eq__(self, other):
        if isinstance(other, Region):
            return self._regionId == other._regionId
        return False

    def __hash__(self):
        return hash(self._regionId)

class Document:
    def __init__(self, doc_id: int, tokens: List[Token], entities: List[Entity], relations: List[Relation],
                 encoding: List[int], char_encoding: List[List[int]], seg_encoding: List[int], regions):
...
class Dataset(TorchDataset):
    ...
    def create_document(self, tokens, entity_mentions, relations, doc_encoding, char_encoding, seg_encoding, regions) -> Document:
        document = Document(self._doc_id, tokens, entity_mentions, relations, doc_encoding, char_encoding, seg_encoding, regions)
        self._documents[self._doc_id] = document
        self._doc_id += 1

        return document

    def create_entity(self, entity_type, tokens, phrase, mapping_region) -> Entity:
        mention = Entity(self._eid, entity_type, tokens, phrase, mapping_region)
    ...
    def create_region(self, image_path, image_id, mapping_region):
        region = Region(self._regionID, image_path, image_id, mapping_region)
        self._regions[self._regionID] = region
        self._regionID += 1
        return region
    ...
    def __getitem__(self, index: int):
        ...
        if self._mode == Dataset.TRAIN_MODE:
            return sampling.create_train_sample(doc, random_mask=self.random_mask_word, tokenizer = self._tokenizer, repeat_gt_entities = self._repeat_gt_entities,
                                                ...
                                                aux_processor=self.aux_processor, rcnn_processor=self.rcnn_processor,
                                                image_path=self.image_path, aux_size=self.aux_size,
                                                rcnn_size=self.rcnn_size, path_num=self.path_num
                                                )
        else:
